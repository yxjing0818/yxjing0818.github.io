...
```text
alpha = (segment_index + t) / segment_count
u0 = u_min + alpha * (u_max - u_min)
```

这种方法非常便宜，适合快速获得一个大致初值。
